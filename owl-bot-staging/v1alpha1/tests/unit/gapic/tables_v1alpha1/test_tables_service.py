...
from google.area120.tables_v1alpha1.services.tables_service import TablesServiceAsyncClient
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (TablesServiceAsyncClient, transports.TablesServiceGrpcAsyncIOTransport, "grpc_asyncio"),
...
@pytest.mark.parametrize("client_class", [
    TablesServiceClient, TablesServiceAsyncClient
])
@mock.patch.object(TablesServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(TablesServiceClient))
@mock.patch.object(TablesServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(TablesServiceAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (TablesServiceClient, transports.TablesServiceGrpcTransport, "grpc", grpc_helpers),
    (TablesServiceAsyncClient, transports.TablesServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
...
def test_list_tables_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, tables.Table)
                   for i in results)
...
def test_list_workspaces_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, tables.Workspace)
                   for i in results)
...
async def test_list_workspaces_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_workspaces(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_list_rows_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        results = list(pager)
        assert len(results) == 6
        assert all(isinstance(i, tables.Row)
                   for i in results)
...
async def test_list_rows_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_rows(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_get_table_rest_interceptors(null_interceptor):
    transport = transports.TablesServiceRestTransport(
        credentials=ga_credentials.AnonymousCredentials(),
        interceptor=None if null_interceptor else transports.TablesServiceRestInterceptor(),
        )
    client = TablesServiceClient(transport=transport)
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_get_table") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_get_table") as pre:
...
def test_list_tables_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_list_tables") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_list_tables") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_get_workspace_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_get_workspace") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_get_workspace") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_list_workspaces_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_list_workspaces") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_list_workspaces") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_get_row_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_get_row") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_get_row") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_list_rows_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_list_rows") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_list_rows") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_create_row_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_create_row") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_create_row") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_batch_create_rows_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_batch_create_rows") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_batch_create_rows") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_update_row_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_update_row") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_update_row") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_batch_update_rows_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "post_batch_update_rows") as post, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_batch_update_rows") as pre:
        pre.assert_not_called()
        post.assert_not_called()
...
def test_delete_row_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_delete_row") as pre:
        pre.assert_not_called()
...
def test_batch_delete_rows_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.TablesServiceRestInterceptor, "pre_batch_delete_rows") as pre:
        pre.assert_not_called()
...
@pytest.mark.parametrize("transport_class", [transports.TablesServiceGrpcTransport, transports.TablesServiceGrpcAsyncIOTransport])
def test_tables_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.TablesServiceGrpcTransport, transports.TablesServiceGrpcAsyncIOTransport])
def test_tables_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
